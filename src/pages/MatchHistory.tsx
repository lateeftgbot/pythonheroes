                ...
                <div className="max-w-6xl mx-auto w-full px-6 sm:px-10 flex-1 flex flex-col min-h-0">
                    ...
                    <div className="mb-6 shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/10 pb-6">
                        ...
                        <div className="flex gap-[1px] bg-white/10 p-[1px]">
                            <div className="px-5 py-2 bg-[#0a0a0a] border border-white/5 flex flex-col items-center min-w-[90px]">
                                <p className="text-[9px] font-bold text-primary uppercase tracking-[0.2em]">TOTAL XP</p>
                                <p className="text-lg font-black">{totalScore}</p>
                            </div>
                            <div className="px-5 py-2 bg-[#0a0a0a] border border-white/5 flex flex-col items-center min-w-[70px]">
                                <p className="text-[9px] font-bold text-secondary uppercase tracking-[0.2em]">PVP</p>
                                <p className="text-lg font-black">{pvpHistory.length}</p>
                            </div>
                            <div className="px-5 py-2 bg-[#0a0a0a] border border-white/5 flex flex-col items-center min-w-[70px]">
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">SOLO</p>
                                <p className="text-lg font-black">{soloHistory.length}</p>
                            </div>
                        </div>
                    </div>

                    {/* Tab Switcher - Square - Fixed Width to prevent shifting */}
                    <div className="flex mb-0 shrink-0 self-start w-full md:w-auto h-12">
                        <button
                            onClick={() => setActiveTab('solo')}
                            className={cn(
                                "flex-1 md:flex-none flex items-center justify-center gap-3 px-8 font-black text-[11px] uppercase tracking-[0.2em] transition-all duration-200 border-t border-l border-r border-white/10",
                                activeTab === 'solo' 
                        ...
                        >
                            ...
                            Solo Training
                            <span className="opacity-50 text-[9px]">[{soloHistory.length}]</span>
                        </button>
                        <button
                            onClick={() => setActiveTab('pvp')}
                            className={cn(
                                "flex-1 md:flex-none flex items-center justify-center gap-3 px-8 font-black text-[11px] uppercase tracking-[0.2em] transition-all duration-200 border-t border-r border-white/10",
                                activeTab === 'pvp' 
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar-brutalist space-y-3">
                            {activeHistory.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-center space-y-6">
                                    <div className="w-20 h-20 border-2 border-dashed border-black/10 flex items-center justify-center grayscale opacity-40">
                                        {activeTab === 'solo' ? <Target className="w-10 h-10" /> : <Swords className="w-10 h-10" />}
                                    </div>
                                    <div className="space-y-2">
                                        <h3 className="text-xl font-black uppercase tracking-tighter italic">LOG_SECTOR_EMPTY</h3>
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {activeHistory.map((record, index) => (
                                        <div 
                                            key={record.id || index}
                                            className="group relative bg-white border-2 border-[#0a0a0a]/5 hover:border-primary p-4 sm:p-5 transition-all duration-200"
                                        >
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                                {/* Left Section */}
                                                <div className="flex items-center gap-4">
                                                    <div className={cn(
                                                        "w-12 h-12 flex items-center justify-center shrink-0 border-2 border-black",
                                                        record.mode === 'Solo' 
                                                            ? "bg-primary text-black" 
                                                            : "bg-secondary text-black"
                                                    )}>
                                                        {record.mode === 'Solo' ? <Target className="w-6 h-6" /> : <Swords className="w-6 h-6" />}
                                                    </div>
                                                ...
                                                {/* Right Section - Score */}
                                                <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 sm:border-l border-black/5 pt-3 sm:pt-0 sm:pl-6">
                                                    <div className="text-right">
                                                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">YIELD_TOTAL</p>
                                                        <p className="text-2xl font-black tracking-tighter text-[#0a0a0a]">
                                                            +{record.score}
                                                            <span className="text-primary text-[10px] ml-1">XP</span>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
